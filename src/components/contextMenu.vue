    ...
    <Teleport to="#ContextMenuGlobalRenderArea">
        <Transition name="ctxmenu-transition">
            <div class='context_menu_container' :style="{ marginTop: coordY + 'px', marginLeft: coordX + 'px' }"
                v-if="isShowing">
                <v-list density="compact" v-for="(list,indexi) in computedMenuLists" >
                    <v-list-item v-for="(item, indexj) in list" :key="index" :value="index"
...
const handleRightClick = async (event) => {
    coordX.value = event.pageX+props.width > document.body.offsetWidth?
                   document.body.offsetWidth - props.width:event.pageX
// TODO 获取菜单高度
