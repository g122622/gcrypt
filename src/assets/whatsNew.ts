...
export default {
    ...
    "2.6.0": [
        ...
        "支持快捷键锁定app",
        "界面优化",
        "去除SQLite二进制库的引用，缩减包体积",
        "app终于有自己的图标了"
    ],
    "3.0.0": [
        "当前任务数可以在顶栏看到了",
        "更新关于页面",
        "将electron升级到27，vue和vuetify相应升级",
        "重写窗口锁定控制逻辑，修复一系列bug，提高安全性",
        "更换了一些图标",
        "侧边栏增加文件管理器",
        "修复文件管理器底部tip无法hover的bug"
    ],
    "3.1.0": ["列表虚拟化，显著提升列表渲染速度", "优化启动速度", "将js文件编译为字节码"],
    "3.2.0": ["界面优化", "优化启动速度", "支持文件复制与剪切"],
    "3.3.0": [
        "修改文件读取逻辑，支持读取mtp设备的文件",
        "支持文件批量复制与剪切",
        "构建过程使用缓存，加快构建速度",
        "核心优化：去除冗余代码，修复移动文件bug",
        "修复移动和复制文件后缩略图没有的问题",
        "支持保存每个文件夹的布局选项"
    ],
    ...
    "3.7.0": [
        "文件管理器支持搜索文件（仅搜索当前目录/深搜）",
        "视频会显示角标",
        "file-item在看图模式下，默认显示的信息由文件日期改为文件大小"
    ],
    "3.8.0": [
